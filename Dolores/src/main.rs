    ...
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        loop {
            ...
            let lastspecies = {
                let data_read = ctx.data.read().await;
                data_read
                    .get::<LastSnepContainer>()
                    .expect("Expected LastSnepContainer in TypeMap.")
                    .clone()
            };
            ...
            if species.lock().await.as_ref().expect("a value")
                == lastspecies.lock().await.as_ref().expect("as value")
            {
                continue;
            }
