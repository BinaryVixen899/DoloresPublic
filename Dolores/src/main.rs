...
impl EventHandler for Handler {
    async fn message(&self, ctx: Context, msg: Message) {
        let responsemessage = match msg.content.as_str() {
            "What does this look like to you" => "Doesn't look like much of anything to me.",
            "Mow" => "Father, I think we're having Kieran for dinner tonight",
            "Skunk Noises" => "No, she is too precious to eat",
            "Chirp" => "Father, I think we're having Kauko for dinner!",
            "Yip Yap!" => "Father, I think I've found dinner. And it's coyote.",
            _ => return, // If Dolores doesn't recognize the prompt, don't do anything
        };
    ...
    async fn ready(&self, ctx: Context, ready: Ready) {
        ...
        let channel = channels
            .into_iter()
            .find(|channel| channel.name == "speciesupdates");
        let specieschannelid = channel.map(|k| k.id.0);
        ...
        loop {
            let lastspecies = species.lock().await.clone();
            ...
            ctx.http
                .send_message(specieschannelid, &map)
                .await
                .unwrap_or_else(|_| {
    ...
    */
    ...
    let futureb = async {
        ...
        while let Some(item) = stream.next().await {
            let Ok(transformation) = item else {
            ...
            if let Some(ref mut species) = *species {
...
async fn discord(species: Arc<Mutex<Option<String>>>) {
    ...
    let Ok(buid) = botuserid.parse::<u64>() else { return };
...
async fn ellenspecies(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    let response = client
        ...
        .send()
        .await?;
    let species = match response.error_for_status() {
...
async fn ellengender(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    let gender = match &genderblock[3] {
        notion::models::Block::Heading1 { heading_1, .. } => {
            let text = &heading_1.rich_text[0];
            text.plain_text()
        }
        _ => "She/Her",
    };
...
async fn get_ellen_species(src: Source) -> Result<String> {
    match src {
        Source::Notion => {
            ...
            let species = if let notion::models::Block::Heading1 { heading_1, .. } = &test[1] {
